...
def sigmoid (x, shift, mult):
    """
    Usando este sigmoid para disuadir ona red por encima de otra
    """
    return 1 / (1 + np.exp(-(x + shift) * mult));

def ge_time_smoothed_representation (
    spectral, samplerate, hop_lenght, time_constant_s = 0.001
):

    t_frames = time_constant * samplerate / float (hop_lenght)

    #Por defecto, esto soluciona la ecuación por b:
    #b**2 + (1 - b) / t_frames - 2 = 0
    #Esto se aproxima a la mitad del ancho máximo al cuadrado de la respuesta de frecuencia bajando y filtrando el IIR
...
def _smoothing_filter (n_grad_freq, n_grad_time):
    ...
    """
    Genera un filtro suave en la mascara de sonido, esto se puede analizar en su espectograma

    Argumentos:
    
    n_grad_freq {[type]} -- [cuantos canales de frecuenta suavizan encima de la mascara de sonido]

    n_grad_time {[type]} -- [cuantos canales de tiempo para suavizar sobre la mascara de sonido]
    """
